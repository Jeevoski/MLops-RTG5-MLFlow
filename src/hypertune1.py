...
with mlflow.start_run() as parent:
    ...
    # Log training data (cast target to float to avoid schema warning)
    train_df = X_train.copy()
    train_df['target'] = y_train
    train_df = mlflow.data.from_pandas(train_df.astype({"target": "float64"}))
    mlflow.log_input(train_df, "training")

    # Log test data
    test_df = X_test.copy()
    test_df['target'] = y_test
    test_df = mlflow.data.from_pandas(test_df.astype({"target": "float64"}))
